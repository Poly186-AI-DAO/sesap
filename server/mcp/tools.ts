...
function stripNullValues(obj: any): any {
  ...
  if (Array.isArray(obj)) return obj.map(stripNullValues).filter((item: any) => item !== undefined);
...
export function registerTools(server: McpServer): void {
    ...
    async ({ transcript }) => {
      if (transcript.length < 100) {
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({ error: 'Transcript must be at least 100 characters' }),
          }],
          isError: true,
        };
      }
      ...
      try {
        console.error(`[SESAP MCP] Generating contract from transcript (${transcript.length} chars)`);
        ...
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({ error: `Contract generation failed: ${message}` }),
          }],
          isError: true,
        };
      }
  ...
  server.tool(
    ...
    'Use this to preview contract output from existing artifacts.',
    {
      ...
      template: z.string().describe('TemplateMark template content (.tem.md file content)'),
      data: z.string().describe('JSON data string with $class properties on every object'),
    },
  ...
  server.tool(
    ...
    async ({ model, template, data }) => {
      try {
        ...
        try {
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                valid: false,
                errors: [`JSON data is not valid JSON: ${parseError instanceof Error ? parseError.message : 'parse error'}`],
              }),
            }],
          };
        }
        ...
        console.error(`[SESAP MCP] Validation result: ${result.success ? 'valid' : 'invalid'}`);
